...
{
  vector< Statement* >* vector_statement = get_statement_stack();
  ...
  else
  {
    ...
    if (debug_level == parser_execute)
    {
      ...
      vector< Statement* >* vector_statement = get_statement_stack();
      Statement* staty = vector_statement->front();
      Osm_Script_Statement* root =
          dynamic_cast< Osm_Script_Statement* >(staty);
          //dynamic_cast< Osm_Script_Statement* >(get_statement_stack()->front());
      if (root)
	root->set_factory(&stmt_factory);
    }
...
void clean_up_core()
{
  /*vector< Statement* >* statements = get_statement_stack();

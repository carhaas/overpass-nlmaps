...
int execute_command(string db_dir, string clone_db_dir, uint log_level, Debug_Level debug_level, int area_level, bool respect_timeout, string* ptr_nlmaps = NULL){
  ...
  try
  {
    ...
    //string test_query = "area[\"name\"=\"Heidelberg\"]->.a;( node(area.a)[\"tourism\"];way(area.a)[\"tourism\"];relation(area.a)[\"tourism\"]; );out;";
    //string xml_raw(get_xml_from_string(error_output, test_query));//prints the encoding remark and gets the input
    string xml_raw(get_xml_console(error_output));//prints the encoding remark and gets the input
    cerr << "xml_raw: " << xml_raw << endl;
  ...
  //   timecounter -= tv.tv_sec + double(tv.tv_usec)/1000000;
    //get_statement_stack prob from scripting_core.cc and returns statement_stack_ which is a vector< Statement* >
...
int main(int argc, char *argv[])
{
  ...
  return execute_command(db_dir, clone_db_dir, log_level, debug_level, area_level, respect_timeout);
}

...
inline uint32 calc_index(const std::vector< uint32 >& node_idxs)
{
  ...
  // | bitwise OR
  if (*it & 0x80000000) // & is bitwise AND
  {
    if (*it & 0x00000001)
    {
...
inline uint32 ilat(uint32 ll_upper, uint32 ll_lower)
{
  uint32 upper = ll_upper & 0xaaaaaaaa;//2863311530
  upper = (upper | (upper<<1)) & 0xcccccccc;//3435973836
  upper = (upper | (upper<<2)) & 0xf0f0f0f0;//4042322160
  upper = (upper | (upper<<4)) & 0xff00ff00;//4278255360
  upper = (upper | (upper<<8)) & 0xffff0000;//4294901760

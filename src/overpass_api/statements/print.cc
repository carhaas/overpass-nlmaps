...
double break_even(double lng){
  double break_point = 0.0;
  if(lng == 0){
    break_point = 180;
  } else if(lng > 0){
    break_point = (180 - lng) * -1;
  } else {
    break_point = 180 + lng;
  }
  return break_point;
...
bool is_east(double l, double x){//long
  double b = break_even(l);
  bool east =  false;
  if(l > 0){
    if((l<x && x<=180) || (b>x && x>=-180)){
      east = true;
    }
  } else {
    if(l<x && x<b){
...
bool is_west(double l, double x){//long
  double b = break_even(l);
  bool west =  false;
  if(l > 0){
    if(b<x && x<l){
...
bool is_south(double l, double x){//lat
  bool south =  false;
  if(l>x){
    south = true;
  }
...
bool is_north(double l, double x){//lat
  bool north =  false;
  if(l<x){
    north = true;
  }
...
void Print_Statement::tags_by_id
  (const map< TIndex, vector< TObject > >& items,
   ...
   Resource_Manager& rman, Transaction& transaction,
   const File_Properties* meta_file_prop, uint32& element_count, NLmaps_query* ptr_query_info)
{
  // order relevant elements by id
  vector< pair< const TObject*, uint32 > > items_by_id;
  for (typename map< TIndex, vector< TObject > >::const_iterator
	...
	return;
      if(ptr_query_info == NULL){
        ...
        OSM_element element;
          ...
          if(lat_lon.first!=-1.0 && lat_lon.second!=-1.0) {
            double& lat = lat_lon.first;
            double& lng = lat_lon.second;
            bool fullfill_cardinal = false;
    				if(find(c.begin(), c.end(), cardinal_direction_t::NORTH) != c.end()){
    					fullfill_cardinal = is_north(ptr_query_info->ref_element.lat_lon.first, lat);
    				}
            if(find(c.begin(), c.end(), cardinal_direction_t::SOUTH) != c.end()){
    					fullfill_cardinal = is_south(ptr_query_info->ref_element.lat_lon.first, lat);
    				}
            if(find(c.begin(), c.end(), cardinal_direction_t::EAST) != c.end()){
    					fullfill_cardinal = is_east(ptr_query_info->ref_element.lat_lon.second, lng);
    				}
            if(find(c.begin(), c.end(), cardinal_direction_t::WEST) != c.end()){
    					fullfill_cardinal = is_west(ptr_query_info->ref_element.lat_lon.second, lng);
    				}
            if(!fullfill_cardinal){ continue;}
          }
				}
        if(ptr_query_info->keys.size()>0){
          vector< pair< string, string > >* tags = &(tags_by_id[items_by_id[i.val()].first->id.val()]);
					stringstream ss_answer_element;
					bool first_element = true;
          for (vector< pair< string, string > >::const_iterator it(tags->begin()); it != tags->end(); ++it){
						for(unsigned int i=0; i<ptr_query_info->keys.size(); i++){
	            if(it->first == ptr_query_info->keys[i]){
	              if(first_element){
                  ss_answer_element << it->second;
                  first_element = false;
                } else {
                  ss_answer_element << " and " << it->second;
                }
	            }
						}
          }
          element.value = ss_answer_element.str();
        }
        if(ptr_query_info->keys_detailed_info.size()>0){
          vector< pair< string, string > >* tags = &(tags_by_id[items_by_id[i.val()].first->id.val()]);
          for (vector< pair< string, string > >::const_iterator it(tags->begin()); it != tags->end(); ++it){
						for(unsigned int i=0; i<ptr_query_info->keys_detailed_info.size(); i++){
	            if(it->first == ptr_query_info->keys_detailed_info[i]){
 				        stringstream ss_answer_element;
                ss_answer_element << it->second;
                element.detailed_info.push_back(ss_answer_element.str());
	            }
						}
          }
        }
        ...
        for (vector< pair< string, string > >::const_iterator it(tags->begin()); it != tags->end(); ++it){
          ...
          if(it->first == "addr:street"){
            element.name = it->second;
          }
          if(it->first == "addr:housenumber"){
            element.name = it->second;
          }
          if(it->first == "addr:postcode"){
            element.name = it->second;
          }
          if(it->first == "addr:city"){
            element.name = it->second;
          }
        }


        ptr_query_info->count_elements++;
        osm_id_and_type(items_by_id[i.val()].second, *(items_by_id[i.val()].first), element.osm_id_number, element.osm_type);
        try{ //always get latlong
          latlong(items_by_id[i.val()].second, *(items_by_id[i.val()].first), &element.lat_lon);
        } catch(...){
          cerr << "Cannot get latitude/longitude from this element to this query: " << ptr_query_info->query << endl;
        }
        ptr_query_info->elements.push_back(element);
      }
    }
  }
}

int Print_Statement::osm_id_and_type(uint32 ll_upper, const Node_Skeleton& skel, string& id, string& type){
  stringstream ss_id;
  ss_id << skel.id.val();
  id = ss_id.str();
  type="N";
...
int Print_Statement::osm_id_and_type(uint32 ll_upper, const Way_Skeleton& skel, string& id, string& type){
  stringstream ss_id;
  ss_id << skel.id.val();
  id = ss_id.str();
  type="W";
...
int Print_Statement::osm_id_and_type(uint32 ll_upper, const Relation_Skeleton& skel, string& id, string& type){
  stringstream ss_id;
  ss_id << skel.id.val();
  id = ss_id.str();
  type="R";
...
int Print_Statement::osm_id_and_type(uint32 ll_upper, const Area_Skeleton& skel, string& id, string& type){
  stringstream ss_id;
  ss_id << skel.id.val();
  id = ss_id.str();
  type="A";
...
int Print_Statement::latlong(uint32 ll_upper, const Node_Skeleton& skel, pair<double, double>* ptr_lat_lon){
  double lat_value = round(lat(ll_upper, skel.ll_lower)*pow(10,10))/pow(10,10);
  double lon_value = round(lon(ll_upper, skel.ll_lower)*pow(10,10))/pow(10,10);
  ptr_lat_lon->first = lat_value;
  ptr_lat_lon->second = lon_value;
	return 0;
}

int Print_Statement::latlong(uint32 ll_upper, const Way_Skeleton& skel, pair<double, double>* ptr_lat_lon){
    std::vector< Quad_Coord > geometry = way_geometry_store->get_geometry(skel);
    Double_Coords double_coords(geometry);
	  const std::pair< Quad_Coord, Quad_Coord* >* bounds = bound_variant(double_coords, Print_Target::PRINT_CENTER);
    double lat_value = round(lat(bounds->first.ll_upper, bounds->first.ll_lower)*pow(10,10))/pow(10,10);
    double lon_value = round(lon(bounds->first.ll_upper, bounds->first.ll_lower)*pow(10,10))/pow(10,10);
    ptr_lat_lon->first = lat_value;
    ptr_lat_lon->second = lon_value;
		return 0;
}

int Print_Statement::latlong(uint32 ll_upper, const Relation_Skeleton& skel, pair<double, double>* ptr_lat_lon){
    std::vector< std::vector< Quad_Coord > > geometry_vector = relation_geometry_store->get_geometry(skel);
    Double_Coords double_coords((geometry_vector));
    const std::pair< Quad_Coord, Quad_Coord* >* bounds = bound_variant(double_coords, Print_Target::PRINT_CENTER);
    double lat_center = round(lat(bounds->first.ll_upper, bounds->first.ll_lower)*pow(10,10))/pow(10,10);
    double lon_center = round(lon(bounds->first.ll_upper, bounds->first.ll_lower)*pow(10,10))/pow(10,10);
    ptr_lat_lon->first = lat_center;
    ptr_lat_lon->second = lon_center;
		return 0;
}

int Print_Statement::latlong(uint32 ll_upper_save, const Area_Skeleton& skel, pair<double, double>* ptr_lat_lon){
	return 0;
}
...
void Print_Statement::execute(Resource_Manager& rman, NLmaps_query* ptr_query_info)
{
  clock_t begin = clock();
  ...
  if (mode & (Print_Target::PRINT_GEOMETRY | Print_Target::PRINT_BOUNDS | Print_Target::PRINT_CENTER) || ptr_query_info!=NULL)
  {
    delete way_geometry_store;
  ...
  if (mode & Print_Target::PRINT_TAGS)
  {
	...
	tags_by_id(mit->second.areas, *area_settings().AREA_TAGS_LOCAL,
		   ...
		   *rman.get_area_transaction(), 0, element_count, ptr_query_info);
    }
    else
    {

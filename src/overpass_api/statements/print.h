  ...
  private:
    ...
    void tags_by_id
      (const map< TIndex, vector< TObject > >& items,
       ...
       Resource_Manager& rman, Transaction& transaction,
       const File_Properties* meta_file_prop, uint32& element_count, NLmaps_query* ptr_query_info = NULL);
    ...
    int latlong(uint32 ll_upper, const Node_Skeleton& skel, pair<double, double>* ptr_lat_lon);
    int latlong(uint32 ll_upper, const Way_Skeleton& skel, pair<double, double>* ptr_lat_lon);
    int latlong(uint32 ll_upper, const Relation_Skeleton& skel, pair<double, double>* ptr_lat_lon);
    int latlong(uint32 ll_upper, const Area_Skeleton& skel, pair<double, double>* ptr_lat_lon);

    int osm_id_and_type(uint32 ll_upper, const Node_Skeleton& skel, string& id, string& type);
    int osm_id_and_type(uint32 ll_upper, const Way_Skeleton& skel, string& id, string& type);
    int osm_id_and_type(uint32 ll_upper, const Relation_Skeleton& skel, string& id, string& type);
    int osm_id_and_type(uint32 ll_upper, const Area_Skeleton& skel, string& id, string& type);
};
